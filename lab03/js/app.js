...
function salvarUsuario() {
  ...
  if (DB.usuarios.find(u => u.email.toLowerCase() === email.toLowerCase()))
    return mostrarErro('usr-erro', 'Já existe um usuário com este e-mail.');
...
function salvarCategoria() {
  ...
  if (DB.categorias.find(c => c.nome.toLowerCase() === nome.toLowerCase()))
    return mostrarErro('cat-erro', 'Já existe uma categoria com este nome.');
...
function renderCursos() {
  ...
  if (!tbody) return;
  const nivelClass = { 'Iniciante': 'success', 'Intermediário': 'warning', 'Avançado': 'danger' };
  tbody.innerHTML = lista.length === 0
    ...
    : lista.map(c => {
        ...
        return `<tr>
          ...
          <td><span class="badge bg-${nivelClass[c.nivel] || 'secondary'}">${c.nivel}</span></td>
...
function renderAulas() {
  ...
  const tipoClass = { 'Vídeo': 'primary', 'Texto': 'info', 'Quiz': 'warning' };
  tbody.innerHTML = lista.length === 0
    ...
    : lista.map(a => {
        ...
        return `<tr>
          ...
          <td>${a.titulo}</td>
          <td><span class="badge bg-${tipoClass[a.tipoConteudo] || 'secondary'}">${a.tipoConteudo}</span></td>
...
function salvarMatricula() {
  ...
  if (tipo === 'curso') {
    ...
    const jaMatriculado = DB.matriculas.find(m => m.idUsuario === Number(idUsuario) && m.idCurso === Number(idCurso));
...
function salvarProgresso() {
  ...
  const jaConc = DB.progressos.find(p => p.idUsuario === Number(idUsuario) && p.idAula === Number(idAula));
...
function verificarConclusao(idUsuario, idCurso) {
  ...
  const concluidas = DB.progressos.filter(p =>
    p.idUsuario === idUsuario && aulasDoCurso.some(a => a.id === p.idAula)
  );
  ...
  if (concluidas.length === aulasDoCurso.length) {
    ...
    // Atualiza data de conclusão da matrícula
...
function salvarAvaliacao() {
  ...
  const jaAvaliou = DB.avaliacoes.find(a => a.idUsuario === Number(idUsuario) && a.idCurso === Number(idCurso));
...
function salvarTrilhaCurso() {
  ...
  const jaExiste = DB.trilhasCursos.find(tc => tc.idTrilha === Number(idTrilha) && tc.idCurso === Number(idCurso));
...
function realizarCheckout() {
  ...
  renderPagamentos();
  renderAssinaturas();
  setTimeout(() => alert(`✅ Pagamento realizado com sucesso!\n\nID da Transação: ${pagamento.idTransacaoGateway}\nMétodo: ${metodo}\nValor: R$ ${plano.preco.toFixed(2)}`), 200);
}
...
function atualizarTodosSelects() {
  ...
  ['cur-categoria','tri-categoria','filtro-categoria-cursos'].forEach(id =>
    popularSelect(id, DB.categorias, c => c.nome, c => c.id, id === 'filtro-categoria-cursos' ? 'Todas as categorias' : 'Selecione...'));
  ...
  ['mat-plano','pay-plano'].forEach(id =>
    popularSelect(id, DB.planos, p => `${p.nome} — R$ ${p.preco.toFixed(2)} (${p.duracaoMeses} meses)`, p => p.id));
...
document.addEventListener('DOMContentLoaded', () => {
  // Tipo de matrícula — mostrar/ocultar campos
  ...
  // Renderizações iniciais (tabelas vazias)
  renderUsuarios(); renderCategorias(); renderCursos();
  renderModulos(); renderAulas(); renderMatriculas();
  renderProgressos(); renderCertificados(); renderAvaliacoes();
  renderTrilhas(); renderTrilhasCursos();
  renderPlanos(); renderPagamentos(); renderAssinaturas();
});
